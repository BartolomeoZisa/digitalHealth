#!/usr/bin/env python3
"""
Mirror Movements classification
--------------------------------
Pipeline:
- Outer test: LOPO (Leave-One-Participant-Out)
- Inner validation: single holdout split on training subjects
...
def session_df_to_tensor(df: pd.DataFrame, feature_cols: List[str]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    X = np.nan_to_num(X)
    X = X[:, np.newaxis, :]  # (n_samples, 1, n_features)
...
def run_lopo_holdout_kmedoids(
    ...
    for fold_idx, (outer_train_idx, outer_test_idx) in enumerate(
        ...
        inner_train_idx, inner_val_idx = make_single_holdout_split(
            X_train_all,
            y_train_all,
            groups_train_all,
        ...
        validation_f1_per_outer_fold.append(float(best_val_f1))
...
def run_lopo_holdout_boss(
    ...
    for fold_idx, (outer_train_idx, outer_test_idx) in enumerate(
        outer.split(X_uni, y, groups), start=1
    ):
        print(f"[BOSSEnsemble] Outer fold {fold_idx}/{n_outer}")
        ...
        clf_val = build_boss_classifier()
        clf_val.fit(X_inner_train, y_inner_train)
        y_val_pred = clf_val.predict(X_inner_val)
        val_f1 = f1_score(y_inner_val, y_val_pred, zero_division=0)
        validation_f1_per_outer_fold.append(float(val_f1))
...
def main() -> None:
    ...
    results["KMedoids_AC_Preprocessing_Euclidean"] = run_lopo_holdout_kmedoids(
        X=X_pre,
        y=y,
        groups=groups,
        metric="euclidean",
    ...
    results["KMedoids_AC_Preprocessing_DTW"] = run_lopo_holdout_kmedoids(
        X=X_pre,
        y=y,
        groups=groups,
        metric="dtw",
    ...
    results["KMedoids_AC_NoPreprocessing_Euclidean"] = run_lopo_holdout_kmedoids(
        X=X_raw,
        y=y,
        groups=groups,
        metric="euclidean",
    ...
    results["KMedoids_AC_NoPreprocessing_DTW"] = run_lopo_holdout_kmedoids(
        X=X_raw,
        y=y,
        groups=groups,
        metric="dtw",
    ...
    if boss_available:
        results["BOSSEnsemble_AC_Preprocessing"] = run_lopo_holdout_boss(
            X_uni=X_boss_pre,
            y=y,
            groups=groups,
        )
        results["BOSSEnsemble_AC_NoPreprocessing"] = run_lopo_holdout_boss(
            X_uni=X_boss_raw,
            y=y,
            groups=groups,
        )
